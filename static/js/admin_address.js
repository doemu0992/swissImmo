...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!el.strasse) {
        console.warn("Feld 'id_strasse' nicht gefunden. Script bricht ab.");
        return;
    }

    // Schwebendes Menü erstellen
    const results = document.createElement('div');
    results.id = 'geoadmin-results';
    ...
    el.strasse.addEventListener('input', function() {
        ...
        // Debounce: Nicht bei jedem Tastendruck sofort feuern
            ...
            fetch(`https://api3.geo.admin.ch/rest/services/api/SearchServer?searchText=${encodeURIComponent(val)}&type=locations&origins=address&limit=10`)
                ...
                .then(data => {
                    ...
                    data.results.forEach(item => {
                        ...
                        // EGID holen (verschiedene Felder prüfen)
                        let rawEgid = null;
                        ...
                        // Zeigt EGID direkt in der Liste an -> Gutes Feedback
                        row.innerHTML = `<strong>${label}</strong><br><span style="color:#666">EGID: ${rawEgid || "?"} | Kt: ${rawKanton}</span>`;

                        row.onclick = function() {
                            // Adresse parsen (Format: "Strasse Nr, PLZ Ort")
                            // Regex sucht nach PLZ (4 Ziffern)
                            ...
                            // WICHTIG: EGID setzen!
                            if (el.egid && rawEgid) {
                                el.egid.value = rawEgid;
                                // Visuelles Feedback: Grüner Rand
                                el.egid.style.border = "2px solid green";
                                el.egid.style.backgroundColor = "#e8f5e9";
                            }
