document.addEventListener('DOMContentLoaded', function() {
    ...
    // =====================================================================
    // 1. ZERSTÖRUNG DER INNENLIEGENDEN UNFOLD-RAHMEN
    // =====================================================================
    const readonlyBoxes = document.querySelectorAll('td .readonly');

    readonlyBoxes.forEach(box => {
        // Tötet den Hintergrund, Rahmen und Schatten
    ...
    // =====================================================================
    // 2. DEINE ABSCHNITTS-BEARBEITUNG (Inline Editing)
    // =====================================================================
    ...
    const fieldsets = document.querySelectorAll('fieldset:not(.map-fieldset)');
    ...
    fieldsets.forEach(fieldset => {
        ...
        readOnlyContainer.className = 'view-mode-container grid grid-cols-1 lg:grid-cols-2 gap-4 py-4';
        ...
        rows.forEach(row => {
            ...
            inputs.forEach((input, index) => {
                ...
                const labelText = labels[index] ? labels[index].innerText : 'Feld';
                ...
                valueDiv.innerHTML = `
                    <div class="text-[11px] font-bold text-gray-500 uppercase tracking-wide mb-1">${labelText}</div>
                    <div class="text-sm font-medium text-gray-900">${valueText}</div>
                `;
        ...
        editBtn.innerHTML = `
            ...
            Bearbeiten
        `;
        editBtn.className = 'text-xs text-indigo-600 hover:text-indigo-800 font-semibold px-3 py-1.5 rounded-md hover:bg-indigo-50 transition-colors shadow-sm ring-1 ring-inset ring-indigo-600/10 cursor-pointer flex items-center';
        titleEl.appendChild(editBtn);

        const actionButtons = document.createElement('div');
        actionButtons.className = 'flex justify-end gap-3 mt-4 pt-4 border-t border-gray-100 hidden';
        actionButtons.innerHTML = `
            <button type="button" class="cancel-btn px-3 py-1.5 text-sm font-semibold text-gray-600 hover:bg-gray-100 rounded-md transition-colors cursor-pointer">
                Abbrechen
            </button>
            <button type="button" class="save-btn px-4 py-1.5 text-sm font-bold rounded-md shadow-sm transition-colors flex items-center gap-2 cursor-pointer" style="background-color: #4f46e5 !important; color: #ffffff !important;">
