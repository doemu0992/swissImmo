...
| **`ORGANISATION_RUECKFALL = True`** an vier Belegarten | `finance/models.py` — `Zahlungseingang`, `DebitorenRechnung`, `Mahnung`, `KreditorenRechnung` | alle Wege optional; „noch nicht zugeordnet" ist im Bankabgleich ein regulärer Arbeitszustand. Greppbar: `grep -rn ORGANISATION_RUECKFALL` |
