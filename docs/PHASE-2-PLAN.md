...
| **`ORGANISATION_RUECKFALL = True`** an **sieben** Modellen in **vier** Apps | `core.Pendenz` · `crm.Kommunikation` · `rentals.Dokument` · `finance.DebitorenRechnung`, `Zahlungseingang`, `Mahnung`, `KreditorenRechnung` | alle Wege optional; „noch nicht zugeordnet" ist ein regulärer Arbeitszustand. **Nicht per `grep` zählen** — über die Registry: `getattr(m, 'ORGANISATION_RUECKFALL', False)`. Der Test `RueckfallBestandTests` hält die Zahl fest |
| **`crm.Vorlage`-Lesezugriffe** brauchen die ODER-Abfrage | `core/services/ticket_workflow.py:118`, `core/services/vorlagen_defaults.py:130`, `core/views/fw/schaeden.py:213`, `core/views/fw/profil.py:645`, `core/views/fw/mietprozess.py:122`, `core/views/fw/kommunikation.py:38` | `NULL` heisst dort „mitgelieferte Systemvorlage". Eine reine `organisation=org`-Abfrage sieht sie **nicht** — sechs Stellen müssen `Q(organisation=org) \| Q(organisation__isnull=True)` fragen, sobald gefiltert wird |
