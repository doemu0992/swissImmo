...
**TS-14 – Ungenutzte Abhängigkeiten und Doppelungen.** ~~`django-jazzmin`, das Google-Cluster, `pytesseract`, `docxtpl`, `weasyprint`, `pyHanko` sind installiert und ungenutzt; `vulture` steht unter den Laufzeitabhängigkeiten.~~ **Weitgehend erledigt (P0.5, 14.08.2026):** acht Pakete entfernt, `vulture` steht in `requirements-dev.txt`, `GEMINI_API_KEY` ist weg. **Eine Angabe hier war falsch:** `pyHanko` ist keine tote Abhängigkeit, sondern eine von `xhtml2pdf` — es bleibt. Offen bleibt der flache pip-freeze mit den verwaisten Transitiven (gehört zu P2.3) und die Doppelung der PDF-Bibliotheken: nach dem Wegfall von `weasyprint` sind es noch drei (`xhtml2pdf`, `reportlab`, `svglib`) plus zwei QR-Bibliotheken (`segno` zum Erzeugen, `zxing-cpp` zum Lesen — die beiden sind keine Doppelung, sie tun Gegenteiliges).
...
|---|---|---|
| ~~P0.1~~ | ~~**`/app/`-SPA entfernen** — entschieden (E1).~~ — **erledigt am 14.08.2026** in E1a/E1b/E1c. Die Prüfung ergab: ausser den zwei öffentlichen Endpunkten (Bewerbungsformular, DocuSeal-Webhook) hatte kein `/api/`-Pfad einen Aufrufer ausserhalb der SPA. | M |
| ~~P0.2~~ | ~~**Unfold-Admin auf lesend stellen** — entschieden (E2).~~ — **erledigt am 14.08.2026.** Alle **27** registrierten Admins (25 eigene plus Djangos `auth.User`/`auth.Group`) und alle **18** Inlines sitzen auf `core/admin_base.NurLesen*`. Die Analyse-Annahme „14 von 25 sind bereits schreibgeschützt" traf nicht zu: Die vorhandenen `has_*_permission`-Sperren sassen fast alle auf **Inlines**, nicht auf den ModelAdmins — schreibgeschützt war einzig `AktivitaetsLogAdmin`. Die Isolation gilt weiterhin. | S |
...
| ~~P0.5~~ | ~~`psycopg` nachtragen; ungenutzte Pakete entfernen; `GEMINI_API_KEY` streichen~~ — **erledigt am 14.08.2026.** `psycopg[binary]==3.2.13` ergänzt (Fassung 3, von Django 5.2 empfohlen — `.env.example` nannte fälschlich `psycopg2-binary`). Acht Pakete entfernt, jedes per Import-Suche belegt: `django-jazzmin`, `vulture`, `pytesseract`, `docxtpl`, `weasyprint`, `google-genai`, `google-ai-generativelanguage`, `google-api-python-client`. **Korrektur zu TS-14:** `pyHanko` ist NICHT ungenutzt — es ist eine Abhängigkeit von `xhtml2pdf` und bleibt. Verwaiste Transitive der entfernten Pakete stehen weiterhin im flachen Freeze; sie sauber aufzulösen braucht pip-tools und gehört zu P2.3. | S |
...
| ~~P0.7~~ | ~~Groq-Nutzung nachträglich formalisieren~~ — **dokumentiert am 14.08.2026** in `docs/GROQ-BELEGERKENNUNG.md`: was gesendet wird (der ganze Beleg, nicht ausgewählte Felder — inkl. möglicher Mieternamen auf Handwerkerrechnungen), der belegte Ausschalter, der fehlende Kostenrahmen und vier offene Entscheide. **Zwei davon sind kaufmännisch/rechtlich und bleiben beim Betreiber:** DPF-Zertifizierung prüfen und Auftragsbearbeitungsvertrag. | S |
| ~~P0.8~~ | ~~`test_foto_beleg_mit_qr_wird_dekodiert` gegen fehlendes `zxing-cpp` absichern~~ — **erledigt am 14.08.2026.** Es waren **zwei** betroffene Tests, nicht einer: `test_upload_qr_bild_erzeugt_saubere_rechnung` hängt an derselben Bibliothek. Beide tragen jetzt `@skipUnless(ZXING_DA, …)`. | XS |
