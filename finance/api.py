...
# ========================================================
# HILFSFUNKTION: STORNO BUCHUNG
# ========================================================
def erstelle_storno_buchung(original_buchung):
...
def run_sollstellung(request, payload: SollstellungSchema):
    ...
    try:
        ...
        return 400, {"success": False, "error": "Systemkonten fehlen (1100, 3000, 3020)."}
    ...
    for v in vertraege:
        ...
        # PRO-RATA BERECHNUNG FÜR TEIL-MONATE
        ...
        if netto > 0:
            Buchung.objects.create(
                ...
                betrag=netto, debitoren_rechnung=rechnung
            )
        if nk > 0:
            Buchung.objects.create(
                ...
                betrag=nk, debitoren_rechnung=rechnung
            )
        erstellt += 1
...
@router.delete("/zahlungen/{zahlung_id}", response={200: dict})
@transaction.atomic
...
@router.delete("/kreditoren/{rechnung_id}", response={200: dict})
@transaction.atomic
...
def create_debitorenrechnung(request, payload: DebitorenRechnungCreateSchema):
    ...
    try:
        ...
        Buchung.objects.create(
            ...
            betrag=rechnung.betrag, debitoren_rechnung=rechnung
        )
    except Buchungskonto.DoesNotExist:
        pass
...
@router.delete("/debitoren-rechnungen/{rechnung_id}", response={200: dict})
@transaction.atomic
...
def create_konto(request, payload: KontoCreateSchema):
    if Buchungskonto.objects.filter(nummer=payload.nummer).exists():
        return 400, {"success": False, "error": "Existiert bereits."}
...
def import_standard_kontenplan(request):
    standard_konten = [
        ...
        {"nummer": "1190", "bezeichnung": "Durchlaufkonto", "typ": "bilanz", "is_hnk_relevant": False},
        ...
        {"nummer": "3020", "bezeichnung": "Nebenkosten Akonto", "typ": "ertrag", "is_hnk_relevant": False},
        {"nummer": "4000", "bezeichnung": "Unterhalt & Reparaturen", "typ": "aufwand", "is_hnk_relevant": False},
        {"nummer": "4100", "bezeichnung": "Heizkosten", "typ": "aufwand", "is_hnk_relevant": True, "standard_verteilschluessel": "m2"},
        {"nummer": "4110", "bezeichnung": "Wasser / Abwasser", "typ": "aufwand", "is_hnk_relevant": True, "standard_verteilschluessel": "m3"},
        {"nummer": "4120", "bezeichnung": "Hauswartung", "typ": "aufwand", "is_hnk_relevant": True, "standard_verteilschluessel": "m2"},
...
def get_erfolgsrechnung(request, liegenschaft_id: Optional[int] = None):
    qs = Buchung.objects.filter(ist_storno=False) # Nur echte Buchungen berücksichtigen
...
def delete_periode(request, periode_id: int):
    get_object_or_404(AbrechnungsPeriode, id=periode_id).delete()
    return 204, None